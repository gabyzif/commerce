...
export default class SentryErrorProvider {
  ...
  exception(error: Error, options: Options) {
    ...
    if (!this.initialized) {
      // Log the error
      console.warn(
        `Tried to report an error but Sentry was not initialized`,
        error,
        options
      )
    ...
    if (error && !error.name) {
      ...
      error.name =
        options?.extras?.message || options?.origin || 'Unknown error'
    }
  ...
  message(message: string, options: Options) {
    ...
    if (!this.initialized) {
      // Log the error
      console.warn(
        `Tried to report a message but Sentry was not initialized`,
        message,
        options
      )
